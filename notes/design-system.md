...
| **`--destructive` (bachelor theme)** | #210: desaturated persimmon (`--accent-heat` desat ~40%), never red | ~~unbound in `[data-theme="bachelor"]` — shadcn's saturated red `oklch(0.577 0.245 27.325)` leaked into every `text-destructive`/`bg-destructive` call site (prod walk: Revoke rendered pomegranate)~~ → `--destructive: oklch(0.62 0.12 35)` in the bachelor block | ✅ **RESOLVED 2026-06-10** (#297). 4.6:1 on card `#1a1517`, 5.0:1 on `#100C0F`. `--destructive-foreground` deliberately NOT bound — note: `edit-item-form.tsx:339` (delete-confirm solid fill) does reference `text-destructive-foreground`, which resolves nowhere today; pre-existing, unchanged by #297. |
| **`:root` token cascade leaks into themes** | §Theming model: a theme block is the *only* token source under its `data-theme` — "One CSS variable swap = one theme" | ~~unscoped `:root {}` meant any token absent from `[data-theme="bachelor"]` silently inherited shadcn's light-mode default (the #297 leak class; live leaks at fix time: `--secondary`/`--accent` rendered near-white `oklch(0.97 0 0)` on /login secondary button, /dates effort badge, dropdown/select item focus, roster button hover)~~ → `:root` block scoped to `:root:not([data-theme])` (nested `&:not()`); consumed leakers bound in the bachelor block: `--secondary(-foreground)` = `--surface-elevated`/`--ink-primary` (follows the `--muted`/`--card` mapping), `--accent(-foreground)` = `--accent-heat-soft` (§Accent "hover ambient") / `--ink-primary` | ✅ **RESOLVED 2026-06-10** (#188). Enumeration of the 17 `:root`-only tokens: 4 bound (above); `--chart-1..5` + 8 `--sidebar-*` left unbound — zero consumers (no chart/sidebar components; sidebar nav is a §AI-tells ban). `.dark` block left unscoped: dead boilerplate, nothing sets `class="dark"`. #180's direct `background-color` on `html[data-theme=bachelor]` **kept** — load-bearing for `e2e/theme-smoke.spec.ts:19`. Note: spec has no bespoke "secondary surface" token; the `--secondary` binding is the elevated-surface mapping precedent, flagged for ratification. |
| error-surface text uses `text-destructive` (persimmon) | #209: error text = ink shade + hairline on `--surface-elevated`, **not** persimmon | ~34 `text-destructive` error-text call sites across `app/` + `components/` (validation lines, `role="alert"` banners) + 1 state-signal misuse (`dates/_celebrant-view.tsx:128` "no-go" chip — §State signals specs `declined` as ink-tertiary) | ⚠️ **DRIFT — pre-existing, logged during the #297 audit.** #210 destructive *verbs* (Revoke / Delete / Remove / Unassign, ~11 sites) correctly keep persimmon. follow-up #301. |
| radius scale generally | polar (2 / 8 / 16 / 24 / 999) | shadcn-default calc: `--radius-sm` `calc(*0.6)`=4.8px … `--radius-4xl`=20.8px, `--radius` = 8px under `[data-theme=bachelor]` (10px in the never-rendered unthemed fallback) | ⚠️ **RE-SCOPED 2026-06-10 (#289 closed re-scoped).** The mandated audit landed: `notes/radius-audit.md` — 101 call-site lines, **46 visually-changing** under the ratified rule (>6 trigger → no CSS / call-site change this wave). Token rebase + call-site rewrite are inseparable (`rounded-lg` is a numeric no-op at 8px today but the named spec value is 16) → one coherent follow-up issue (**#304**), gated on ratifying 2 spec gaps: pill-CTA buttons (16 sites) and error-banner radius (4 sites, #209-silent, adj. #301). |
